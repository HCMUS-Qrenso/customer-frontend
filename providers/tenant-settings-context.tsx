...
import type { TenantSettings, TaxSettings, ServiceChargeSettings, OperatingHours, OrderSettings } from '@/lib/types/table';
import { formatDateTime as formatDateTimeLib, formatShortDate, formatTime as formatTimeLib, formatRelativeDate as formatRelativeDateLib } from '@/lib/format';
...
export function TenantSettingsProvider({
  ...
  const isOpenNow = useMemo(() => {
    return (): boolean => {
      ...
      if (typeof todayHours.isOpen === 'boolean') {
        if (!todayHours.isOpen || !todayHours.slots || todayHours.slots.length === 0) {
          return false;
        }
  ...
  const getTodayHours = useMemo(() => {
    return (): string | null => {
      ...
      // Legacy format
      if (todayHours.closed || !todayHours.open || !todayHours.close) return null;
  ...
  const formatDate = useMemo(() => {
    return (dateString: string): string => {
      return formatShortDate(dateString, { timezone: effectiveSettings.timezone });
    };
  }, [effectiveSettings.timezone]);

  const formatTime = useMemo(() => {
    return (dateString: string): string => {
      return formatTimeLib(dateString, { timezone: effectiveSettings.timezone });
    };
  }, [effectiveSettings.timezone]);

  const formatDateTime = useMemo(() => {
    return (dateString: string): string => {
      return formatDateTimeLib(dateString, { timezone: effectiveSettings.timezone });
    };
  }, [effectiveSettings.timezone]);
